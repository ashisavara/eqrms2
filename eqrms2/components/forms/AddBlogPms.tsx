...
export function AddBlogPmsForm() {
    ...
    const onSubmit = handleSubmit(async (data) => {
        ...
        try {
            ...
            await supabaseInsertRow('blogs_pms', processedData);

            if (typeof window !== "undefined") {
                toast.success("PMS Blog created successfully!");
                setTimeout(() => {
                    router.push('/internal/public-site/pmsblog');
                }, 1500);
            }
        } catch (error) {
            console.error('Error creating PMS Blog:', error);
            if (typeof window !== "undefined") {
                toast.error("Failed to create PMS Blog. Please try again.");
            }
            setIsLoading(false);
        }
    });
        ...
        <form onSubmit={onSubmit} className="w-full p-4 space-y-4">
            ...
            <div className="flex justify-end">
                <Button type="submit" disabled={isLoading}>
                    {isLoading ? 'Creating...' : 'Create PMS Blog'}
                </Button>
            </div>
        </form>
    );
