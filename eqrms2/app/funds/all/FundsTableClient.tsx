...
export default function FundsTableClient({ data, pagination, filterOptions, filterConfig, searchColumns }: FundsTableClientProps) {
  ...
  const config: ServerTablePageConfig<RmsFundsScreener> = {
    ...
    columns: [
      { key: 'fund_name', header: 'Fund Name', align: 'left', render: (value, row) => <Link href={`/funds/${row.slug}`} className="text-blue-600 font-bold hover:underline">{value}</Link> },
      { key: 'fund_rating', header: 'Fund', render: (value) => <RatingDisplay rating={value} /> },
      { key: 'fund_performance_rating', header: 'Perf', render: (value) => <RatingDisplay rating={value} /> },
      { key: 'amc_rating', header: 'AMC', render: (value) => <RatingDisplay rating={value} /> },
      { key: 'structure_name', header: 'Structure' },
      { key: 'cat_long_name', header: 'Category' },
      { key: 'estate_duty_exposure', header: 'Estate Duty' },
      { key: 'us_investors', header: 'US Investors' },
      { key: 'one_yr', header: '1 Year', render: (value) => value !== null ? <ComGrowthNumberRating rating={value} /> : '-' },
      { key: 'three_yr', header: '3 Year', render: (value) => value !== null ? <ComGrowthNumberRating rating={value} /> : '-' },
      { key: 'five_yr', header: '5 Year', render: (value) => value !== null ? <ComGrowthNumberRating rating={value} /> : '-' },
      { key: 'since_inception', header: 'Since Inception', render: (value) => value !== null ? <ComGrowthNumberRating rating={value} /> : '-' }
    ],
